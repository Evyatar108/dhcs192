...
def analyze_text(text) -> Tuple[TextAnalysis, Dict]:
    ...
    coreferences_clusters = __extract_coreferences_clusters(data)
    relations = __extract_openie_relations(data)
...
def __extract_sentimented_sentences(data) -> List[SentimentedSentence]:
...
def __extract_tagged_entities(data) -> List[TaggedTextEntity]:
    tagged_tokens = []
...
def __pred_same_tagged_entity(prev_tagged_token: TaggedTextEntity, curr_tagged_token: TaggedTextEntity):
    return curr_tagged_token.tag != 'O'


def __generate_clusters(list, pred_same_cluster):
    cluster = []
    prev_elem = None
    for elem in list:
...
def __extract_coreferences_clusters(data) -> List[List[CoReference]]:
...
def __extract_openie_relations(data) -> List[Relation]:
    relations = [
